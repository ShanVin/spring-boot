...
for (( i = 0; i < $2; i++ )); do
    ...
    java -Djava.ext.dirs=../lib -jar ../lib/$application_name-*.jar --server.port=${server_port} --logging.file.path=${logpath} --spring.config.location=file:../env/application-$1.yml >> $outfile 2>&1 &
